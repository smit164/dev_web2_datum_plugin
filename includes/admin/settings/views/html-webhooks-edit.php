			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_name">
						...
						<?php
						/* translators: %s: date */
						echo dm_help_tip( sprintf( __( 'Friendly name for identifying this webhook, defaults to Webhook created on %s.', 'woocommerce' ), strftime( _x( '%b %d, %Y @ %I:%M %p', 'Webhook created on date parsed by strftime', 'woocommerce' ) ) ) ); // @codingStandardsIgnoreLine
						?>
					</label>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_status">
						...
						<?php echo dm_help_tip( __( 'The options are &quot;Active&quot; (delivers payload), &quot;Paused&quot; (does not deliver), or &quot;Disabled&quot; (does not deliver due delivery failures).', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
					<select name="webhook_status" id="webhook_status" class="dm-enhanced-select">
						<?php
						$statuses       = dm_get_webhook_statuses();
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_topic">
						...
						<?php echo dm_help_tip( __( 'Select when the webhook will fire.', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
					<select name="webhook_topic" id="webhook_topic" class="dm-enhanced-select">
						<?php
							$topic_data = DM_Admin_Webhooks::get_topic_data( $webhook );
			...
			<tr valign="top" id="webhook-action-event-wrap">
				<th scope="row" class="titledesc">
					<label for="webhook_action_event">
						...
						<?php echo dm_help_tip( __( 'Enter the action that will trigger this webhook.', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_delivery_url">
						...
						<?php echo dm_help_tip( __( 'URL where the webhook payload is delivered.', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_secret">
						...
						<?php echo dm_help_tip( __( 'The secret key is used to generate a hash of the delivered webhook and provided in the request headers.', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
			...
			<tr valign="top">
				<th scope="row" class="titledesc">
					<label for="webhook_api_version">
						...
						<?php echo dm_help_tip( __( 'REST API version used in the webhook deliveries.', 'woocommerce' ) ); ?>
					</label>
				</th>
				<td class="forminp">
					<select name="webhook_api_version" id="webhook_api_version">
						<?php foreach ( array_reverse( dm_get_webhook_rest_api_versions() ) as $version ) : ?>
						...
						if ( $webhook->get_id() ) :
							$delete_url = wp_nonce_url(
								add_query_arg(
									...
									), admin_url( 'admin.php?page=dm-settings&tab=advanced&section=webhooks' )
								), 'delete-webhook'
							);
							?>
