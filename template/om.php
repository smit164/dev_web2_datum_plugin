...
if(isset( $_GET['p_id'] ) && $_GET['p_id'] !='') {
    ...
    if ( is_null($single_property->CAPdfDocument) || empty($single_property->CAPdfDocument) || $datum_user->Id == 0) {
        echo '
    ...
    $printable = '
        <div>
            ...
            <table class="OEPL_table" style="width: 100%;">
                <tr>
                    <td colspan="2"><strong>LISTING NAME: </strong>'.ucwords(strtolower($single_property->Name)).'</td>
                </tr>
                <tr>
    ...
    $pdf->SetImportUse();
    $pagecount = $pdf->SetSourceFile($upload_dir['basedir'].'/property_ca_document/'.$single_property->ID.'/'.$single_property->CAPdfDocument);

    if(!$pagecount) {
        $ilovepdf = new Ilovepdf(DATUM_ILOVEPDF_KEY,DATUM_ILOVEPDF_SECRET_KEY);
        $myTask = $ilovepdf->newTask('repair');
        $file1 = $myTask->addFile($upload_dir['basedir'].'/property_ca_document/'.$single_property->ID.'/'.$single_property->CAPdfDocument);
        $myTask->execute();
        $wprootpath = DM_ABSPATH;
        $myTask->download($wprootpath.'/Ilovepdf_files/');
        if(file_exists( $wprootpath.'/Ilovepdf_files/'.$single_property->CAPdfDocument )) {
            unlink($upload_dir['basedir'].'/property_ca_document/'.$single_property->ID.'/'.$single_property->CAPdfDocument);
            copy ($wprootpath.'/Ilovepdf_files/'.$single_property->CAPdfDocument, $upload_dir['basedir'].'/property_ca_document/'.$single_property->ID.'/'.$single_property->CAPdfDocument);
            unlink($wprootpath.'/Ilovepdf_files/'.$single_property->CAPdfDocument);
        }
    }
