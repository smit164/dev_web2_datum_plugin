...
class DM_Query {
	...
	public function init_query_vars() {
		...
		$this->query_vars = array(
			// Checkout actions.
			'property_listing_id'        => get_option( 'datum_property_listing_id', 'property_listing_id' ),
			'login_page_id'              => get_option( 'datum_login_page_id', 'login_page_id' ),
			// My account actions.
			'myaccount_page_id'          => get_option( 'datum_myaccount_page_id', 'myaccount_page_id' ),
			'terms_page_id'              => get_option( 'datum_terms_page_id', 'terms_page_id' ),
		);
	}
	...
	public function get_endpoint_title( $endpoint, $action = '' ) {
		...
		switch ( $endpoint ) {
			case 'order-pay':
				$title = __( 'Pay for order', 'datum' );
				break;
			case 'order-received':
				$title = __( 'Order received', 'datum' );
				break;
			case 'orders':
				if ( ! empty( $wp->query_vars['orders'] ) ) {
					/* translators: %s: page */
					$title = sprintf( __( 'Orders (page %d)', 'datum' ), intval( $wp->query_vars['orders'] ) );
				} else {
					$title = __( 'Orders', 'datum' );
				}
				break;
			case 'view-order':
				$order = dm_get_order( $wp->query_vars['view-order'] );
				/* translators: %s: order number */
				$title = ( $order ) ? sprintf( __( 'Order #%s', 'datum' ), $order->get_order_number() ) : '';
				break;
			case 'downloads':
				$title = __( 'Downloads', 'datum' );
				break;
			case 'edit-account':
				$title = __( 'Account details', 'datum' );
				break;
			case 'edit-address':
				$title = __( 'Addresses', 'datum' );
				break;
			case 'payment-methods':
				$title = __( 'Payment methods', 'datum' );
				break;
			case 'add-payment-method':
				$title = __( 'Add payment method', 'datum' );
				break;
			case 'lost-password':
				if ( in_array( $action, array( 'rp', 'resetpass', 'newaccount' ) ) ) {
					$title = __( 'Set password', 'datum' );
				} else {
					$title = __( 'Lost password', 'datum' );
				}
				break;
			default:
				$title = '';
		 ...
		 * @param string $action Optional action or variation within the endpoint.
		 */
		return apply_filters( 'datum_endpoint_' . $endpoint . '_title', $title, $endpoint, $action );
	}
	...
	public function get_endpoints_mask() {
		if ( 'page' === get_option( 'show_on_front' ) ) {
			...
			$myaccount_page_id = get_option( 'datum_myaccount_page_id' );
			$checkout_page_id  = get_option( 'datum_checkout_page_id' );
	...
	public function get_query_vars() {
		return apply_filters( 'datum_get_query_vars', $this->query_vars );
	}
	...
	public function pre_get_posts( $q ) {
		...
		if ( $this->is_showing_page_on_front( $q ) ) {
			...
			if ( $this->page_on_front_is( dm_get_page_id( 'shop' ) ) ) {
				$_query = wp_parse_args( $q->query );
				if ( empty( $_query ) || ! array_diff( array_keys( $_query ), array( 'preview', 'page', 'paged', 'cpage', 'orderby' ) ) ) {
					...
					if ( current_theme_supports( 'datum' ) ) {
		...
		if ( current_theme_supports( 'datum' ) && $q->is_page() && 'page' === get_option( 'show_on_front' ) && absint( $q->get( 'page_id' ) ) === dm_get_page_id( 'shop' ) ) {
			...
			dm_maybe_define_constant( 'SHOP_IS_ON_FRONT', true );

			// Get the actual WP page to avoid errors and let us use is_front_page().
			// This is hacky but works. Awaiting https://core.trac.wordpress.org/ticket/21096.
			global $wp_post_types;

			$shop_page = get_post( dm_get_page_id( 'shop' ) );
	...
	public function handle_get_posts( $posts, $query ) {
		if ( 'product_query' !== $query->get( 'dm_query' ) ) {
			return $posts;
		}
	...
	public function wpseo_metadesc() {
		return WPSEO_Meta::get_value( 'metadesc', dm_get_page_id( 'shop' ) );
	}
	...
	public function wpseo_metakey() {
		return WPSEO_Meta::get_value( 'metakey', dm_get_page_id( 'shop' ) );
	}
	...
	public function product_query( $q ) {
		...
		$q->set( 'dm_query', 'product_query' );
		$q->set( 'post__in', array_unique( (array) apply_filters( 'loop_shop_post_in', array() ) ) );

		// Work out how many products to query.
		$q->set( 'posts_per_page', $q->get( 'posts_per_page' ) ? $q->get( 'posts_per_page' ) : apply_filters( 'loop_shop_per_page', dm_get_default_products_per_row() * dm_get_default_product_rows_per_page() ) );
		...
		do_action( 'datum_product_query', $q, $this );
	}
	...
	public function get_catalog_ordering_args( $orderby = '', $order = '' ) {
		...
		if ( ! $orderby ) {
			// phpcs:ignore WordPress.Security.NonceVerification.Recommended
			$orderby_value = isset( $_GET['orderby'] ) ? dm_clean( (string) wp_unslash( $_GET['orderby'] ) ) : dm_clean( get_query_var( 'orderby' ) );

			if ( ! $orderby_value ) {
				if ( is_search() ) {
					$orderby_value = 'relevance';
				} else {
					$orderby_value = apply_filters( 'datum_default_catalog_orderby', get_option( 'datum_default_catalog_orderby', 'menu_order' ) );
				}
			}
		...
		return apply_filters( 'datum_get_catalog_ordering_args', $args, $orderby, $order );
	}
	...
	public function price_filter_post_clauses( $args, $wp_query ) {
		 ...
		 */
		if ( dm_tax_enabled() && 'incl' === get_option( 'datum_tax_display_shop' ) && ! dm_prices_include_tax() ) {
			$tax_class = apply_filters( 'datum_price_filter_widget_tax_class', '' ); // Uses standard tax class.
			$tax_rates = DM_Tax::get_rates( $tax_class );

			if ( $tax_rates ) {
				$current_min_price -= DM_Tax::get_tax_total( DM_Tax::calc_inclusive_tax( $current_min_price, $tax_rates ) );
				$current_max_price -= DM_Tax::get_tax_total( DM_Tax::calc_inclusive_tax( $current_max_price, $tax_rates ) );
			}
		}

		$args['join']   = $this->append_product_sorting_table_join( $args['join'] );
		$args['where'] .= $wpdb->prepare(
			' AND dm_product_meta_lookup.min_price >= %f AND dm_product_meta_lookup.max_price <= %f ',
			$current_min_price,
			$current_max_price
		);
		return $args;
	...
	public function order_by_price_asc_post_clauses( $args ) {
		$args['join']    = $this->append_product_sorting_table_join( $args['join'] );
		$args['orderby'] = ' dm_product_meta_lookup.min_price ASC, dm_product_meta_lookup.product_id ASC ';
		return $args;
	}
	...
	public function order_by_price_desc_post_clauses( $args ) {
		$args['join']    = $this->append_product_sorting_table_join( $args['join'] );
		$args['orderby'] = ' dm_product_meta_lookup.max_price DESC, dm_product_meta_lookup.product_id DESC ';
		return $args;
	}
	...
	public function order_by_popularity_post_clauses( $args ) {
		$args['join']    = $this->append_product_sorting_table_join( $args['join'] );
		$args['orderby'] = ' dm_product_meta_lookup.total_sales DESC, dm_product_meta_lookup.product_id DESC ';
		return $args;
	}
	...
	public function order_by_rating_post_clauses( $args ) {
		$args['join']    = $this->append_product_sorting_table_join( $args['join'] );
		$args['orderby'] = ' dm_product_meta_lookup.average_rating DESC, dm_product_meta_lookup.rating_count DESC, dm_product_meta_lookup.product_id DESC ';
		return $args;
	}

	/**
	 * Join dm_product_meta_lookup to posts if not already joined.
	 *
	...
	private function append_product_sorting_table_join( $sql ) {
		...
		if ( ! strstr( $sql, 'dm_product_meta_lookup' ) ) {
			$sql .= " LEFT JOIN {$wpdb->dm_product_meta_lookup} dm_product_meta_lookup ON $wpdb->posts.ID = dm_product_meta_lookup.product_id ";
		}
		return $sql;
	}
	...
	public function get_meta_query( $meta_query = array(), $main_query = false ) {
		...
		return array_filter( apply_filters( 'datum_product_query_meta_query', $meta_query, $this ) );
	}
	...
	public function get_tax_query( $tax_query = array(), $main_query = false ) {
		...
		$product_visibility_terms  = dm_get_product_visibility_term_ids();
		...
		if ( 'yes' === get_option( 'datum_hide_out_of_stock_items' ) ) {
		...
		return array_filter( apply_filters( 'datum_product_query_tax_query', $tax_query, $this ) );
	}
			...
			if ( ! empty( $_GET ) ) {
				foreach ( $_GET as $key => $value ) {
					if ( 0 === strpos( $key, 'filter_' ) ) {
						$attribute    = dm_sanitize_taxonomy_name( str_replace( 'filter_', '', $key ) );
						$taxonomy     = dm_attribute_taxonomy_name( $attribute );
						$filter_terms = ! empty( $value ) ? explode( ',', dm_clean( wp_unslash( $value ) ) ) : array();

						if ( empty( $filter_terms ) || ! taxonomy_exists( $taxonomy ) || ! dm_attribute_taxonomy_id_by_name( $attribute ) ) {
							continue;
						}

						$query_type                                    = ! empty( $_GET[ 'query_type_' . $attribute ] ) && in_array( $_GET[ 'query_type_' . $attribute ], array( 'and', 'or' ), true ) ? dm_clean( wp_unslash( $_GET[ 'query_type_' . $attribute ] ) ) : '';
						self::$chosen_attributes[ $taxonomy ]['terms'] = array_map( 'sanitize_title', $filter_terms ); // Ensures correct encoding.
						self::$chosen_attributes[ $taxonomy ]['query_type'] = $query_type ? $query_type : apply_filters( 'datum_layered_nav_default_query_type', 'and' );
					}
				}
			}
		}
	...
	public function layered_nav_init() {
		dm_deprecated_function( 'layered_nav_init', '2.6' );
	}
	...
	public function get_products_in_view() {
		dm_deprecated_function( 'get_products_in_view', '2.6' );
	}
	...
	public function layered_nav_query( $deprecated ) {
		dm_deprecated_function( 'layered_nav_query', '2.6' );
	}
	...
	public function search_post_excerpt( $where = '' ) {
		dm_deprecated_function( 'DM_Query::search_post_excerpt', '3.2.0', 'Excerpt added to search query by default since WordPress 4.5.' );
		return $where;
	}
	...
	public function remove_posts_where() {
		dm_deprecated_function( 'DM_Query::remove_posts_where', '3.2.0', 'Nothing to remove anymore because search_post_excerpt() is deprecated.' );
	}
}
